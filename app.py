...
def set_stock_quantity(product_id, new_qty, date, note="Корректировка остатка"):
    """Устанавливает точный остаток и записывает разницу в историю."""
    row = _t("sm_products").select("quantity,cost_usd").eq("id", product_id).execute().data
    if not row:
        raise ValueError("Товар не найден.")

    current_qty = int(row[0]["quantity"] or 0)
    new_qty = int(new_qty)
    ...
    _t("sm_stock_log").insert({
        ...
        "date": date,
        "delta": delta,
        "cost_usd": row[0].get("cost_usd") or 0,
        "note": note or "Корректировка остатка",
    }).execute()
...
def delete_product(pid):
    """Удаляет товар и его складскую историю.

    Товар, который уже использовался в заказах, не удаляем, чтобы
    не разрушить историю продаж.
    """
    order_items = (
        _t("sm_order_items")
        .select("order_id")
    ...
    if order_items:
        raise ValueError(
            "Этот товар уже использовался в заказах и не может быть удалён. "
            "Это защищает историю продаж."
        )

    # Сначала удаляем движения склада, иначе FK может запретить
    # удаление записи из sm_products.
    _t("sm_stock_log").delete().eq("product_id", pid).execute()
...
with tab_stock:
    ...
    with st_a:
        ...
        if rows:
            ...
            # Keep the ID together with the row instead of relying on
            # the row position/name (names may be duplicated).
            row_by_id = {r["id"]: r for r in rows}
            display_rows = [{k: v for k, v in r.items() if k != "id"} for r in rows]
            st.dataframe(pd.DataFrame(display_rows), use_container_width=True, hide_index=True)

            with st.expander("✏️ Редактировать остаток"):
                edit_options = list(row_by_id.keys())
                edit_id = st.selectbox(
                    "Товар",
                    edit_options,
                    format_func=lambda pid: (
                        f"{row_by_id[pid]['Бренд']} — {row_by_id[pid]['Товар']} "
                        f"(сейчас: {row_by_id[pid]['Остаток']} шт.)"
                    ),
                    key="edit_stock_sel",
                )
                edit_row = row_by_id[edit_id]
                new_stock = st.number_input(
                    "Новый остаток, шт.",
                    min_value=0,
                    value=int(edit_row["Остаток"]),
                    step=1,
                    key=f"edit_stock_qty_{edit_id}",
                )
                edit_note = st.text_input(
                    "Причина изменения (необязательно)",
                    placeholder="Например: пересчёт склада",
                    key=f"edit_stock_note_{edit_id}",
                )

                if st.button("💾 Сохранить остаток", type="primary", key="save_stock_edit"):
                    try:
                        delta = set_stock_quantity(
                            edit_id,
                            new_stock,
                            dt.date.today().isoformat(),
                            edit_note or "Ручная корректировка остатка",
                        )
                        if delta == 0:
                            ...
                            st.success(
                                f"Остаток изменён: {direction}{delta} шт. "
                                f"→ теперь {new_stock} шт."
                            )
                        st.rerun()
                    except Exception as e:
                        st.error(f"Не удалось изменить остаток: {e}")
            ...
            with st.expander("🗑️ Удалить товар"):
                del_id = st.selectbox(
                    ...
                    format_func=lambda pid: (
                        f"{row_by_id[pid]['Бренд']} — {row_by_id[pid]['Товар']} "
                        f"(остаток: {row_by_id[pid]['Остаток']} шт.)"
                    ),
                    key="del_sel",
                )
                del_name = row_by_id[del_id]["Товар"]

                if st.button("Удалить навсегда", type="primary", key="delete_product_btn"):
                    try:
...
# ════════════════════════════════════════════════════════
with tab_orders:
    ord_list_tab, ord_add_tab, ord_store_tab = st.tabs(
        ["📋 Все заказы", "➕ Новый заказ", "🏪 Расчёт с магазином"])
    ...
    # ── Расчёт с магазином ───────────────────────────────
    with ord_store_tab:
        st.subheader("Расчёт с магазином")
        st.caption(
            "Внесите, сколько единиц каждого товара продал магазин с прошлого "
            "расчёта. При сохранении остаток на складе уменьшится, а выручка "
            "и прибыль автоматически попадут в «Финансы» — точно так же, как "
            "при обычном заказе."
        )

        store_products = get_products()
        store_in_stock = [p for p in store_products if p["quantity"] > 0]

        if not store_in_stock:
            st.info("Нет товаров с остатком на складе для расчёта.")
        else:
            sc1, sc2 = st.columns(2)
            with sc1:
                store_name = st.text_input(
                    "Название магазина / точки", value="Магазин",
                    key="store_settle_name")
            with sc2:
                store_date = st.date_input(
                    "Дата расчёта", value=dt.date.today(), key="store_settle_date")

            filter_brand_store = st.selectbox(
                "Фильтр по бренду", ["Все"] + [b["name"] for b in brands],
                key="store_settle_brand_filter")

            visible_products = [
                p for p in store_in_stock
                if filter_brand_store == "Все" or p["brand_name"] == filter_brand_store
            ]

            with st.form("store_settlement_form"):
                st.markdown("**Продано с прошлого расчёта:**")
                sold_inputs = {}
                for p in visible_products:
                    row_c1, row_c2, row_c3 = st.columns([4, 1, 2])
                    with row_c1:
                        st.markdown(
                            f"{p['brand_name']} — {p['name']}  \n"
                            f"<span style='color:#8a94a6;font-size:13px;'>"
                            f"остаток: {p['quantity']} шт. · продажа ${p['sell_usd']:.2f}</span>",
                            unsafe_allow_html=True,
                        )
                    with row_c2:
                        sold_inputs[p["id"]] = st.number_input(
                            "Продано", min_value=0, max_value=int(p["quantity"]),
                            step=1, key=f"store_sold_{p['id']}", label_visibility="collapsed")
                    with row_c3:
                        st.markdown(" ")

                store_note = st.text_input(
                    "Примечание к расчёту (необязательно)", key="store_settle_note")

                submitted = st.form_submit_button(
                    "✅ Провести расчёт", type="primary", use_container_width=True)

                if submitted:
                    sold_items = [
                        p for p in visible_products
                        if sold_inputs.get(p["id"], 0) > 0
                    ]
                    if not sold_items:
                        st.error("Укажите количество проданных единиц хотя бы для одного товара.")
                    else:
                        oid = add_order(
                            client=store_name.strip() or "Магазин",
                            city="", phone="",
                            order_date=store_date.isoformat(),
                            delivery_date=store_date.isoformat(),
                            status="Доставлен",
                            expense_usd=0,
                            notes=store_note or "Расчёт с магазином",
                        )
                        total_units = total_revenue = total_cost = 0
                        for p in sold_items:
                            qty = int(sold_inputs[p["id"]])
                            add_order_item(
                                oid, p["id"],
                                f"{p['brand_name']} — {p['name']}",
                                qty, p["cost_usd"], p["sell_usd"],
                            )
                            total_units   += qty
                            total_revenue += qty * p["sell_usd"]
                            total_cost    += qty * p["cost_usd"]

                        st.success(
                            f"Расчёт #{oid} сохранён: {total_units} шт. продано, "
                            f"выручка ${total_revenue:,.2f}, "
                            f"прибыль ${total_revenue - total_cost:,.2f}. "
